...
const uint8_t SPI_CS1 = 48; // CS pin in SPI mode Rod 3
const uint8_t SPI_CS2 = 46; // CS pin in SPI mode Rod 2
const uint8_t SPI_CS3 = 49; // CS pin in SPI mode Rod 1
...
void rollsinmove() {
  ...
  if (now - prev >= interval) {
    prev = now;
    h1 = a * sin(b * now+3.1415) + a;
    h2 = a * sin(b * now+1.0471) + a;
    h3 = a * sin(b * now) + a;
    motor1.setTargetPosition(h2);
    motor2.setTargetPosition(h3);
    motor3.setTargetPosition(h1);
    //Serial.println(h);
